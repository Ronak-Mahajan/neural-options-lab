...
- **Deep hedging on real paths.** Replaying the same three hedgers over 385 SPY and 567 BTC-USD rolling 30-day windows of real daily closes with ex-ante vol, the simulated tail advantage does not transfer (at 10 bp of cost plain delta has the better CVaR₉₅: SPY 0.027 vs 0.041), while the cost efficiency does (at 50 bp the deep policy has the better mean P&L, −0.67% vs −1.36%, on 69% of windows, with slightly wider tails). [docs/hedging_real_paths.md](docs/hedging_real_paths.md).
...
- The served 0DTE pricing ensemble is not arbitrage-free by construction: on its 134,100-point audit grid it violates the butterfly condition on 4.9% of the box, the calendar condition on 10.5%, and prices below intrinsic on 6.8%. The dashboard's implied-vol surface is the separately trained constrained network, which has zero violations on the same grid; `POST /api/price` serves the raw ensemble and does not yet flag the below-intrinsic region ([docs/no_arbitrage_surface.md](docs/no_arbitrage_surface.md)).
