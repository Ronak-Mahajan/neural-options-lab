...
**Live demo: [neural-options-lab.onrender.com](https://neural-options-lab.onrender.com)**. It runs on an always-on paid instance, so there is no wake-up delay on first load. Every price, Greek and simulation on the dashboard is computed live by the models described below; the accuracy panel reads the committed evaluation in `artifacts/eval.json`.

A neural network that prices arithmetic Asian options 165x faster per price than the 200,000-path Monte Carlo reference it is scored against and 27.9x more accurately than the Levy (1992) closed-form approximation over the full trained box (95% interval 22.9 to 33.5; on price alone Curran's 1994 conditioning approximation is more accurate below σ = 0.55; see [docs/approximation_benchmark.md](docs/approximation_benchmark.md)), wrapped in an interactive dashboard that runs locally with the commands under [Quickstart](#quickstart).
...
- **Pricing.** The neural surrogate prices an arithmetic Asian option in 3.2 ms against the 529 ms its 200,000-path Monte Carlo reference takes, both timed in the same single-threaded run by `scripts/benchmark_approximations.py` ([docs/approximation_benchmark.md](docs/approximation_benchmark.md)); absolute times are specific to the machine and run that produced them, the ratio between the rows is not. Price RMSE is 1.33 basis points of strike on 600 held-out points against 200,000-path references (`artifacts/eval.json`). Over the full trained box its RMSE ratio against Levy (1992) moment matching is 27.9 (95% interval 22.9 to 33.5); at σ = 0.25 the margin is about 5x, and Curran (1994) is more accurate than both there.
- **Variance reduction.** On an at-the-money one-year call at 20% vol, adding a geometric-Asian control variate to antithetic sampling cuts the Monte Carlo standard error by about 24x: 22x to 26x at 5,000 paths and 23x to 24x at 20,000, over two sets of 300 seeded replications, measured as the ratio of empirical standard deviations. The factor depends on the contract; at three other points in the trained box (60% vol; an 80/100 moneyness three-month call at 30% vol; a 130/100 two-year call at 70% vol) it is 5x to 10x. `scripts/variance_reduction.py` regenerates these factors into `docs/variance_reduction.json`.
- **0DTE.** The rough Bergomi ensemble serving maturities of 12 trading days or less is calibrated to the live SPY smile: the served checkpoint carries the accepted 2026-08-20 fit (η 3.657, ρ −0.628, H 0.255; 1.568 vol points over 618 quotes, adopted in commit `03cbc88`), recorded in `artifacts/rough_calibration_20260820.json` and named by the provenance note `train_0dte.py` stamps into the checkpoint metadata alongside a `calibrated` flag. `dataset_0dte.py` reads that same file, so re-running the documented recipe regenerates the dynamics the served model was trained on. That artifact is a record read out of the checkpoint's own metadata, not a re-fit: the optimiser inputs behind it were not preserved, so the fields it cannot attest to are null. Its recorded ensemble validation RMSE is 3.3 bps of strike against its 20,000-path training labels; the arbitrage audit, which re-prices the served checkpoint against 4 × 400,000-path rough Bergomi references, measures 1.9-3.8 bps of price RMSE at its hardest smiles (1-5 days, 10% vol) and 0.8-2.3 bps on the rest, with a largest single-strike error of 14.4 bps ([docs/no_arbitrage_surface.md](docs/no_arbitrage_surface.md)). A held-out evaluation against high-precision references, comparable to `artifacts/eval.json` for the main pricer, is still to be re-measured for this checkpoint; see [the 0DTE section](#the-0dte-model-driver-live-calibration-and-what-is-measured).
...
- **Deep hedging under rough volatility.** On a rough Bergomi + jumps measure with transaction costs, a policy trained under those dynamics reaches a lower CVaR₉₅ than a vol-matched delta hedge from 50 bp of cost (404 ± 7 vs 493 ± 11 bp of strike) and than a Whalley-Wilmott band from 100 bp, at a third of the delta hedge's turnover, which is where most of the gap comes from: 78.5 of those 88.5 bp is a smaller commission bill and 10.0 bp is tail shape, and on demeaned tail alone the Whalley-Wilmott band is ahead below 200 bp. Evaluated on Black-Scholes paths the same policy loses. The ± figures are bootstrap standard errors over one set of 15,000 evaluation paths for one trained network. Under the served-run protocol, policies trained on four seeds and evaluated on two independent path sets put the 50 bp advantage over delta between 48 and 99 bp, with the policy ahead in all eight cases (table under "Training-seed spread" in [docs/deep_hedging_regimes.md](docs/deep_hedging_regimes.md)). Caveat: the measure's parameters come from `artifacts/rough_calibration.json`, the 2026-08-21 SPY fit whose own quality gate marks it `accepted: false` (η at the 4.0 bound); the served 0DTE checkpoint carries a different, accepted fit (H 0.255). The rejection is a caveat on the dynamics, not on the paired comparison. Full grid with standard errors in [docs/deep_hedging_regimes.md](docs/deep_hedging_regimes.md).
- **Deep hedging on real paths.** Replaying a learned CVaR policy (the GAN-measure checkpoint, not one the dashboard serves), a delta hedge and a Whalley-Wilmott band over 384 SPY and 567 BTC-USD rolling 30-day windows of real daily closes with ex-ante vol, the simulated tail advantage does not appear. At 10 bp of cost plain delta has the lower CVaR₉₅ on both assets (SPY 0.027 vs 0.041; BTC 0.057 vs 0.087); under a moving-block bootstrap that respects the window overlap the gap is resolved on BTC and is a point estimate on SPY, whose 95% interval includes zero. The cost efficiency carries over on SPY: at 50 bp the deep policy has the better mean P&L, −0.65% vs −1.35%, on 69% of windows (paired 95% interval 0.44% to 0.92%), trading half as much, with a CVaR₉₅ no lower than delta's. On BTC the 50 bp mean difference is unresolved. Measured 2026-08-20 (commit `5cdd02c`) and replayed 2026-09-11 with the output committed. [docs/hedging_real_paths.md](docs/hedging_real_paths.md).
- **Rough-volatility skew.** The at-the-money skew of listed SPY expiries steepens toward expiry with exponent −0.249 ± 0.033, against the rough-volatility prediction H − ½ = −0.239 at H = 0.261; the rough Bergomi engine reproduces the law in its asymptotic regime and steepens beyond it at the calibrated vol-of-vol. Model exponents come from Monte Carlo at the project's 50 time steps; the linked doc measures that setting's time-step bias (2% of the skew at 1 day, 10% at 126 days at the calibrated vol-of-vol) and the slope extrapolated to zero step size. Same caveat as above: H = 0.261 and η = 3.9 are the gate-rejected `rough_calibration.json` parameters, while the served checkpoint's accepted fit has H = 0.255 (H − ½ = −0.245, also within one standard error of the market exponent). Measurement and figure in [docs/atm_skew_term_structure.md](docs/atm_skew_term_structure.md).
- **Joint (H, η) refit.** Adding the per-expiry ATM skew to the calibration objective moves rough Bergomi's optimum to H = 0.31, ρ = −0.50, forward vol 12.5 % (bootstrap H 0.308 ± 0.011) and puts the Monte Carlo skew exponent at −0.264 ± 0.007, half a market SE from −0.249, for 0.45 vol points of smile RMSE; at the gate-rejected `rough_calibration.json` parameters above the exponent is −0.330, 2.5 SE away. Those ± figures are Monte Carlo error only, at the project's 50 time steps. The time-step bias of that setting is measured at the `rough_calibration.json` parameters, where it moves the 1-45-day slope by +0.010 ([docs/atm_skew_term_structure.md](docs/atm_skew_term_structure.md), Section 5, caveat 4); it is not measured at the refit's parameters. Profile, Pareto front and validation in [docs/joint_skew_refit.md](docs/joint_skew_refit.md).
...
Arithmetic Asian options have no exact closed-form price. The payoff depends on the average price over the option's life, so the standard way to value one is Monte Carlo simulation, which is accurate but slow: in the committed benchmark run the 400,000-path reference takes 946 milliseconds for a single price, and a trading desk needs thousands of prices and their risk sensitivities (the Greeks) refreshed continuously.

A neural network trained on Monte Carlo prices learns the pricing function itself. Once trained it prices the same contract in about 4 milliseconds on the same machine and returns all five Greeks as exact derivatives of the network through automatic differentiation, not finite differences. Price plus all Greeks together costs roughly twelve times a price-only call, because gamma needs a second backward pass through five ensemble members. That turns a batch job into something interactive.

The interesting part is doing this with enough numerical care that the surrogate's error is known rather than assumed: sub-2-basis-point pricing error, delta and vega measured against pathwise Monte Carlo references and theta and rho against their own analytic checks, and a separate model for the short-dated regime where the usual assumptions break down. Gamma is measured against a conditional-Monte-Carlo density reference on the same held-out set: 140 × 10⁻⁴ RMSE per unit of (S/K)² against a reference whose own noise is 50 × 10⁻⁴ RMS, and on the 222 contracts where gamma exceeds 0.5 the median relative error is 0.46% against a median reference standard error of 0.35%, so the comparison resolves gamma to about that floor. The error concentrates at σ√T < 0.1 (356 × 10⁻⁴ RMSE there, against 104 × 10⁻⁴ of reference noise). No experiment in this project has yet hedged a path with the surrogate's own Greeks.

## What it does

The dashboard has four tabs.

**Quote.** Set the contract in the side panel, or load a ticker to take its spot, one-year realised volatility and a Treasury rate. The model price reprices as the inputs move, with all five Greeks per share or for the whole position, next to a fresh Monte Carlo price on a new seed with its 95% interval and the gap between the two in basis points of strike. Type a premium and **Solve** returns the volatility that reproduces it on the served model. The quote copies to the clipboard or downloads as CSV. Below it: the Monte Carlo estimate against path count, an Integrated Gradients attribution of the price, the out-of-sample error distribution, response times on the serving machine, a strike-and-expiry price grid, and the arbitrage-checked short-dated implied-vol surface. Above 12 trading days the contract is an arithmetic Asian; at or below, the rough-volatility 0DTE model prices a European option.
...
The first install takes the CPU build of PyTorch, which is all the app uses; on Linux a plain `pip install -r requirements.txt` would pull the CUDA build and its GPU libraries instead. The torch specifier matches the one in `requirements.txt`, so the second install keeps it.
...
The Desk note's language model is optional. Set `GROQ_API_KEY` (free tier at console.groq.com) to enable it; without it the rule-based summary is used. Loading a ticker needs network access to Yahoo Finance through yfinance and no key.
...
| Piece | Approach |
|---|---|
| Monte Carlo engine | Antithetic sampling with a geometric-Asian control variate (Kemna and Vorst, 1990). On an at-the-money one-year call at 20% vol the control variate cuts the standard error of the antithetic estimator by about 24x (22x to 26x across two sets of 300 seeded replications at 5,000 paths, 23x to 24x at 20,000), measured as the ratio of empirical standard deviations rather than from a reported standard error, which would treat antithetic pairs as independent. The factor is contract-dependent: 5x to 10x at three other measured points (60% vol; an 80/100 moneyness three-month call at 30% vol; a 130/100 two-year call at 70% vol) and 48x to 55x on a three-month at-the-money call at 20% vol. `scripts/variance_reduction.py` regenerates every factor into `docs/variance_reduction.json`. |
...
The 1.4 bps that the previous output head measured reads like a label noise floor and is not
one. It is a systematic offset with a locatable cause.
...
Two changes were tested at full scale (500,000 labels, 5,000 paths each, 400 epochs,
...
worse and vega 1.2% worse.** That is a real regression: conditioning the output
head helps the level and slightly hurts the shape, which is what you would expect
from changing where the magnitude lives in a network trained on a joint
price-and-derivative loss. The promotion is kept because price accuracy is this
model's primary claim, but a service that hedges off these Greeks should weigh
that differently.

The gate reports delta and vega alongside price. They are reported rather than
blocking, and the reason is written into the script.
...
*Residual over geometric* does **not** work, and that is a result in its own right. Since
AM-GM gives `C_arith ≥ C_geo` pathwise, learning only the residual should shrink what the
Softplus floor can distort. It is 37% worse instead, because the residual has a 3.8x wider
relative dynamic range (p99/p50 of 12.46 versus 3.27) and that outweighs the 21x smaller
output scale.

### Closed-form baselines: Levy and Curran

Arithmetic Asians have had fast closed-form approximations since the early 1990s, so the
surrogate is measured against them as well as against Monte Carlo.
`scripts/benchmark_approximations.py --lhs` scores the served ensemble, Levy (1992) /
Turnbull-Wakeman moment matching and Curran's (1994) conditioning approximation on 300
Latin-hypercube points over the full trained box (seed 1992, used by no training or
model-selection draw) against 200,000-path control-variate references, stores every
per-point price in `docs/approximation_benchmark.json`, and renders
[docs/approximation_benchmark.md](docs/approximation_benchmark.md) from that file:

| method | RMSE | bias | p95 abs err |
|---|---|---|---|
| neural ensemble | 1.395 bps | +0.308 | 3.312 |
| Levy moment matching | 38.911 bps | +17.662 | 99.335 |
| Curran, exact threshold | 2.347 bps | −1.038 | 4.859 |
| Monte Carlo, 200k paths | (reference; RMS SE 0.835 bps) | n/a | n/a |

The Levy to ensemble RMSE ratio is 27.9, with a 95% interval of 22.9 to 33.5 from a paired
bootstrap over the 300 points. The ratio is an average over volatility, because Levy's error
is a bias that grows with σ²T: by σ band it is 1.8 (0.05 to 0.30), 14.0 (0.30 to 0.55) and
37.8 (0.55 to 0.80), and at a more typical σ = 0.25 the margin is about 5x (0.72 against
3.65 bps of mean absolute error on the 36-cell grid).

The ordering against Curran depends on volatility. Curran has the lower RMSE for σ below
0.55 (0.188 against 1.180 bps over the 0.05-0.30 band, 0.863 against 1.211 over 0.30-0.55)
and the ensemble above it (1.725 against 3.969 bps over 0.55-0.80), where the gap between
Curran's lower bound and the true price widens with σ²T; over the whole box the ensemble to
Curran RMSE ratio is 0.59 (95% interval 0.50 to 0.74). On the 36-cell grid at σ = 0.25
Curran is 6.9x more accurate than the ensemble on price at about a tenth of the cost per
price. The surrogate adds all five Greeks as exact derivatives from the same pass, batch
throughput, and a training recipe that carries over to dynamics with no conditioning closed
form, such as the rough-volatility 0DTE ensemble.

Both runs time every pricer in the same single-threaded process. On the run that produced
the committed artifact one ensemble price took 3.21 ms against 529.36 ms for its
200,000-path reference, a ratio of 165, with Levy at 299 µs and Curran at 388 µs; the
artifact records the machine's load and power state next to those times. The speed
comparison is not iso-accuracy, since that Monte Carlo price carries an RMS standard error
of 0.835 bps against the ensemble's 1.395 bps RMSE. A re-run at the recorded seed
reproduces every accuracy digit exactly and moves only the timing columns, so the ratios
between rows are the quantity to quote.

Levy has the lower RMSE where the reference price is below 1 bp of strike (0.049 against
0.396 bps over 19 of the 300 points). Where the true price is near zero Levy returns a
price near zero, and the ensemble's Softplus output cannot, the same floor
`scripts/fullscale_ablation.py` documents.
...
price-only call at 3.21 ms (`torch.set_num_threads(1)`, median of 20 calls per point over
the 300 box points; 4.00 ms on the 36-cell grid run) next to its own Monte Carlo references
at 529.36 ms for 200,000 paths and 946.06 ms for 400,000. The artifact stores the
system-wide CPU load and the power source of each timed run next to the numbers they
affect. Re-running at the recorded seeds reproduces every accuracy column digit for digit
and moves every latency column with the machine and its load, so quote the ratios, not the
absolute times.
...
regenerates it, so no absolute figure is quoted for it here. Batch throughput is reported
live by `POST /api/benchmark`, measured on whichever machine is serving, rather than pinned
to a number in this file.
...
paragraph cannot be re-measured, and there is no GPU-versus-CPU serving comparison to quote:
serving is CPU-only in every committed path (`PricingEngine` and `MapPricer` both load
`map_location="cpu"`).
...
### The 0DTE model: driver, live calibration, and what is measured

**The driver.** Rough Bergomi is driven by the Riemann-Liouville Volterra process
`W̃_t = √(2H)∫₀ᵗ(t−s)^(H−½)dW_s`, and `rough_vol.py` simulates it from the exact joint
Gaussian law of the driving Brownian motion and the Volterra integral, verified against
quadrature to 5.4e-08 and against 400,000 draws. The choice of driver decides the short
end. The Type-I (Mandelbrot-Van Ness) fractional Brownian covariance
`0.5(t_i^2H + t_j^2H − |t_i−t_j|^2H)` is the natural thing to reach for and agrees with
the Volterra process only on the diagonal, where both give `Var[W̃_t] = t^2H`, so the
martingale property holds and the paths look plausible: at H = 0.1172 its maximum
off-diagonal relative difference is **4.93**, and it puts `corr(W̃_t1, W̃_t50)` at
**+0.320** against a true **+0.054**. Nor is `chol(C)` the Volterra kernel, because W̃ is
a continuous stochastic integral rather than a linear function of n coarse increments.
Factorising C alone forces `corr(Z_1, W̃_t1) = 1` where the truth is
...
staleness; in the market-closed branch, time-to-expiry is stamped from the quote session
rather than from `now`, a convention that on synthetic quotes with known truth is worth 21%
of √ξ and 0.021 of H. It was retrained twice on 2026-08-20: first on the accepted 2026-08-10 fit
(η 2.688, ρ −0.328, H 0.104; 677 quotes, 8 expiries, 1.553 vol points; ensemble validation
RMSE 3.8 bps, commit `36d0bfb`), then on the accepted 2026-08-20 15:47 EDT fit (η 3.657,
ρ −0.628, H 0.255; 618 quotes, 1.568 vol points; ensemble validation RMSE 3.3 bps, commit
`03cbc88`). The ten days between the two fits moved η from 2.69 to 3.66 and H from 0.104
to 0.255, the regime drift that motivated the CPU pricing map described under
[Repository layout](#repository-layout). Since `36d0bfb`, `train_0dte.py` derives the
...
|---|---|
| ensemble validation RMSE vs 20,000-path training labels | 3.3 bps of strike (recorded at training, `03cbc88`) |
...
| held-out RMSE and bias vs 200,000-path references (the `eval.json` protocol) | to be re-measured |

Validation RMSE against noisy training labels overstates the true error: on an earlier
short-dated checkpoint the gap was 2.4x, 3.5 bps validation against 1.48 bps measured
against 500,000-path references. That is why this project scores against high-precision
references rather than against its own training targets. The re-measurement for the
served checkpoint is the
...
Benchmarking a learned hedger against a delta hedge is easy to get wrong in three specific
ways. This evaluation controls for all three, and each control is worth a measured amount.

1. **The measure has to be risk-neutral.** Standardising generated log-returns per time
   step pins each step's marginals and leaves cross-step covariance free, which is enough
   to realize 1.28x the requested volatility and to break the martingale property of
   discounted spot (E[S_T] above e^{rT} by up to 147 bps); an option booked at
   Black-Scholes is then worth about 30% more under the measure actually simulated.
   `risk_neutralize` enforces terminal variance and the martingale condition exactly, and
   the premium is priced under the simulated measure.
2. **The baseline has to see the same volatility.** A delta hedge run at the requested σ
   while paths realize 1.28σ is a handicapped baseline, and vol-matching alone accounts
   for about 83% of the gap such a comparison reports.
3. **The comparison has to be out of sample.** The WGAN generator is mode-collapsed
   (participation ratio 4.66 of 30 factors), which makes its paths forecastable: R² =
   0.8755 regressing future returns on realized ones, against 0.0006 for GBM. Minimizing
   CVaR on those paths rewards market timing rather than hedging, so the GAN measure is
   not used as an evaluation measure. Full write-up in
   [`docs/hedging_findings.md`](docs/hedging_findings.md).

With all three controlled, and a cost-aware Whalley-Wilmott band added alongside the
vol-matched delta hedge, evaluated out of sample on risk-neutral GBM over a 12-cell
(σ, cost) grid, 15,000 paths per cell:

| policy | beats vol-matched delta | median ratio | beats Whalley-Wilmott |
|---|---|---|---|
| trained on the risk-neutralised GAN measure | 2 / 12 | 1.469 | 0 / 12 |
...
specified baseline.** `compare()` reports both measures side by side with bootstrap
...
tail advantage does not transfer, and the cost efficiency does on SPY
...
backend/
  quant/
    ...
    solve_vol.py          Implied volatility by bisection on the served surrogate (POST /api/implied-vol)
    gamma_reference.py    Conditional Monte Carlo gamma reference that evaluate.py scores against
...
scripts/                  The experiments behind every docs/ page, plus tooling:
                            ...
                            atm_skew_term_structure.py, joint_skew_refit.py,
                            no_arbitrage_surface.py, asian_arbitrage_audit.py,
                            benchmark_approximations.py, btc_full_surface.py, promote_model.py;
                            and the studies behind quoted figures: variance_reduction.py,
                            hedger_seed_spread.py, hedge_real_paths_block.py,
                            heston_step_convergence.py, rb_step_convergence.py
...
data/                     Recorded option surfaces and pricing-map training shards (sources and terms in data/README.md)
...
`d3159e1`). The same end-to-end validation was then repeated out to longer maturities: a
...
(2.457) to 0.03 vol points (commit `67fd862`; `scripts/validate_pricing_map.py`,
...
is an optimizer basin miss, replayed from the archived capture and marked as such in the
series.
...
directories holding 482 `.npz` files of training labels (132 MB). Sources, capture windows and
terms for each dataset are in [data/README.md](data/README.md). The clone is
...
`main`, so the stream grows without enlarging a code clone and nothing about the archive
...
# Hedging policies (one per served dynamics; each writes artifacts/hedger_<measure>.pt)
python -m backend.quant.hedging --measure rbergomi_jumps --iters 4000
python -m backend.quant.hedging --measure gbm --iters 6000
...
There is also a drift monitor (`backend/quant/drift_monitor.py`): one command that re-prices the deployed 0DTE surrogate against a live SPY chain and, if the error crosses a threshold, runs the recalibrate-and-retrain pipeline with promotion gated on the tests. It is a command you run rather than a loop that runs itself: nothing schedules it, no drift log is committed, and the error it reports is not yet a clean drift measurement (see *Scope and limitations*). The scheduled automation this project does run is the surface recorder above, which banks the data any future drift study will be measured on.
...
| `POST /api/implied-vol` | Volatility that reproduces a quoted premium: bisection on the served model over its trained volatility range |
...
| `POST /api/hedge` | Learned CVaR policy, delta hedge and Whalley-Wilmott band on the same 15,000 simulated paths, with bootstrap standard errors and P&L distributions |
| `POST /api/explain` | Integrated Gradients attributions |
| `POST /api/risk-report` | Streamed desk note: Llama 3.1 through Groq when `GROQ_API_KEY` is set, rule-based otherwise |
...
`GET /api/model-info` carries a `zero_dte` block read straight out of `artifacts/model_0dte.pt`: the `calibrated` flag, the fitted η, ρ and H, the Volterra kernel stamp, the note naming the accepted calibration, and the commit and date that last touched the checkpoint. Nothing in it is hardcoded in the service, so the badge on the dashboard cannot claim a calibration the weights do not carry. The block is built once at import, because the commit lookup shells out to `git` and that does not belong on a request path in a 512 MB container; the git fields are `null` where there is no checkout, which is the case in the deployed image.

In the 0DTE regime (`maturity <= 12/252`, a European contract) `POST /api/price` and `/ws/stream` also return the no-arbitrage floor, `max(S − Ke^(−rT), 0)` for a call and `max(Ke^(−rT) − S, 0)` for a put, as `intrinsic`, with `below_intrinsic` and the shortfall in bps of strike. Prices are floored at zero; where a parity-derived put is negative before the floor, the response carries `clamped: true`, and `below_intrinsic` and the shortfall are measured on the value before the floor. See the arbitrage bullet under *Scope and limitations*. The Asian regime's lower bound is a different quantity and these fields are `null` there.

## Scope and limitations

This is a research and portfolio project, not production trading infrastructure. Nothing here is investment advice.

- Market data comes from yfinance, which is retail-grade and has stale or missing quotes, and is used under Yahoo's terms (see [data/README.md](data/README.md)); the Deribit chain the dashboard reads is a committed snapshot, not a live feed. The recorded surface archive under `data/` covers four days (2026-08-20 to 2026-08-23), so there is no out-of-sample-in-time calibration result yet.
...
- The rough Bergomi model has constant parameters. `calibrate.py` fits η, ρ, H and the forward variance ξ jointly (all four are free, H over (0.01, 0.5)), but none of them varies with maturity, so one fit cannot span both the 0DTE range and 3-56 days: the wide-surface fit chooses η 2.12 / H 0.39 and sacrifices the short end that a 17-day fit prices at about 1.5 vol points with η 3.9 / H 0.25 (commit `67fd862`). The calibrator also measures time in ACT/365 calendar days while the surrogate trains on k/252 trading days; the shortest quotes are dropped so no fitted maturity leaves the trained domain, but the two conventions are not yet reconciled (`calibrate.py`, module docstring).
- The served 0DTE pricing ensemble is not arbitrage-free by construction: on its 134,100-point audit grid it violates the butterfly condition on 4.9% of the box, the calendar condition on 10.5%, and prices below intrinsic on 6.8%. The dashboard's implied-vol surface is the separately trained constrained network, which has zero violations on the same grid. `POST /api/price` and the tick stream serve the ensemble price floored at zero, flag with `clamped` a parity-derived put that the floor lifts, and report the European floor next to the price (`intrinsic`, `below_intrinsic`, and the shortfall in bps of strike, measured before the zero floor), so the region is visible rather than silent: a 1-day 1.10-moneyness call, for example, comes back at 10.0044 against a floor of 10.0159, flagged 1.15 bps under. That is disclosure, not a repair: the violation rates above are unchanged ([docs/no_arbitrage_surface.md](docs/no_arbitrage_surface.md)).
- The 0DTE accuracy figures are a training-time validation RMSE and the arbitrage audit's smile-level measurement; the held-out evaluation against 200,000-path references that `artifacts/eval.json` provides for the main pricer has not been re-run on the served checkpoint.
- The deep hedger beats its baselines only under rough volatility with transaction costs, in simulation, on a measure whose calibration record failed its own quality gate; on Black-Scholes paths it loses, and the checkpoint replayed on real SPY and BTC history keeps its cost efficiency on SPY and shows no tail advantage on either asset.
...
The source code is released under the MIT License; see [LICENSE](LICENSE). The licence covers the code and the files this project generates. It does not cover the third-party market data committed under `data/surfaces/` and in `artifacts/deribit_snapshot_btc_20260805T214815Z.json`: SPY option quotes from Yahoo Finance retrieved through yfinance, and BTC and ETH market data from Deribit's public API. Those files remain subject to their providers' terms and are not relicensed. [data/README.md](data/README.md) lists every dataset with its source, capture window and terms, and [NOTICE](NOTICE) states the same scope.
