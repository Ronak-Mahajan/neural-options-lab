...
Under rough Bergomi the at-the-money implied-vol skew
...
the Monte Carlo model's skew departs from a single power law over 1-45 trading
...
cause is the size of the calibrated vol-of-vol. At eta = 3.94,
...
Produced by `python -m scripts.atm_skew_term_structure` in 451 s of wall-clock on 16 CPU
threads, no GPU. Numbers are in `docs/atm_skew_term_structure.json`; the figure is
`docs/atm_skew_term_structure.png`.
...
New York. The file records `accepted: false` ("eta = 3.9352 is pinned at its
bound [0.5, 4]"). The parameters are used because they are the project's latest SPY
calibration file, and the consequence of that pinned eta is the main result of Section 2.
...
- Prices are inverted with `backend.quant.calibrate.implied_vol` on (spot, rate); the
  engine drifts the spot at `rate` with no dividend, so fwd_pv = spot. The skew is
  psi = (iv(+h) - iv(-h)) / 2h.
...
forward F = fwd_pv e^{r tau}. Measuring against fwd_pv would shift k by r tau = 0.0015 at
tau = 0.04, which is comparable to the one-day stencil step. tau is ACT/365 from the pricing
time, the same convention the calibration used for its own Monte Carlo, so model T and
market tau are on the same footing; the top axis "trading days" is T x 252.
...
(0.5% at 1 d, 6% at 126 d) is the larger error beyond 2 days, so the table carries a
Richardson-extrapolated column. (The first-order column is the Bergomi-Guyon leading-order
prediction psi = rho eta sqrt(2H) / (2 (H+1/2)(H+3/2)) T^(H-1/2), C = -0.569; it reduces to
rho eta/4 at H = 1/2. It is reported as the small-eta reference; no claim is made about
...
With SEs this small, the chi^2/dof of 58 to 406 on the first, third and fourth rows shows
that a single straight line does not describe the curve. Local exponents over sliding
windows (central difference):
...
### Mechanism: the calibrated eta is outside the small-vol-of-vol regime
...
| fit | b | SE(b) | note |
|---|---|---|---|
| pooled, all 56 rows (tau 0.0082-0.0414 y) | -0.2492 | 0.0332 | LS SE 0.0039 inflated by sqrt(chi^2/dof), chi^2/dof 71; the scatter across captures sets the error |
...
14:30 -0.258 +- 0.045; 15:45 -0.263 +- 0.019. Thursday afternoon's three exponents (mean
-0.307) are all steeper than Friday's five (mean -0.241). The spread across captures
(sd 0.040) exceeds five of the eight per-capture SEs, and the eight exponents have a
chi^2 of 18.6 on 7 degrees of freedom about their weighted mean (p = 0.01), so part of
the spread is movement of the exponent between captures.
...
- Slope. Model -0.321 +- 0.007 (1-45 d) vs market -0.249 +- 0.033, 2.1 sigma apart. Like
  ...
  H - 1/2 over 2-8 d and 11.5 sigma over 1-45 d.

Over the 2-10-day window the archive covers, the market's short-end skew follows a power
law with exponent about -0.25 (pooled chi^2/dof 71, caveat 5). The rough Bergomi model with
H = 0.26 matches its level, but the eta = 3.94 that the fit required, at its bound, bends
the model's term structure away from the market's. The calibration gate rejects the same
fit for that pinned eta.

---

## 4. BTC (Deribit): market skew and the committed calibration
...
- 2026-08-05 21:48Z: psi = +0.107 +- 0.041 at 1.0 trading-day-equivalent, then negative and
  ...
  -0.103 at 223 d. |psi| peaks at 1-2 weeks, so the front end is the flatter part of the
  curve.
...
wrong sign) inside a week on 08-20/21. The BTC panel therefore supports no rough-volatility
comparison. The market's short-end skew is not a power law, and the committed parameters
...
1. Flat forward variance curve. The model uses xi(t) = xi. Its ATM vol then falls from
   ...
   while the market's ATM vol on 08-20 rises from 8.9% at 2.8 d to 12.1% at 10.4 d. A
   term structure of xi would change the level of psi; whether it changes the exponent is
   untested here.
2. H taken from a rejected calibration. H = 0.2613 comes from a fit that pinned eta
...
5. Quote resolution. The book is tight: inside the
   ...
   per expiry and its residual RMS is 0.09-0.50 vp, which is 2.5 to 17.6 times the in-band
   half-spread (median 7 times). The scatter the market fit sees is therefore smile shape
   the quadratic does not capture, and the 1.74-vp calibration RMSE is roughly 36
   half-spreads wide. The pooled chi^2/dof of 71 says the
   ...
   spread sets the quoted SE; the book's own resolution is far finer. The per-expiry psi SE
   is 0.006-0.07 because it is scaled by those residuals. Nothing in the fit depends on the
   half-spread scale, because the WLS weights are 1/half_spread^2 up to a common factor and
   the SEs are residual-scaled. The band-sensitivity question this raises (how much of psi
   is the quadratic's reach and how much the market's slope) is open.
