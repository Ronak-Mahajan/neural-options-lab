...
smile RMSE (1.31 to 1.76, pricing-map RMSE, mean of three captures). An interior-η solution
...
joint objective on all three captures, because the smile term gains more than the skew term gives back:
...
to 2.01 with `η` free, and from 1.64 to 1.60 inside `z < 2`. The direction of §3.3 survives, and the
