...
| (e) Monte Carlo, full-truncation Euler (Lord, Koekkoek & van Dijk 2010), 200,000 paths, 400 steps, `K = 90/100/110`, `r = 2%` | `T = 1`: `z = -0.42, +0.43, +0.92`; `T = 0.5`: `z = -0.23, +0.51, +1.25` (test bound: 3 SE). At 200 steps (`dt = 0.0025`) the scheme's O(dt) bias reaches `1.4-1.8 SE` at `K = 110` for these Feller-violating parameters, so the test uses 400 |
